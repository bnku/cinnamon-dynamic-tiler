    ...
    static calculateChainTransitions(activeId, direction, config, activeWindows, allVisibleSpans = [], calculateNextStateFn, getDefaultStateFn) {
        ...
        if (!activeWin) {
            const siblingSpans = allVisibleSpans.length > 0
                ? allVisibleSpans
                : activeWindows.map(w => ({
                    hSpan: w.state.hSpan || GridSpans_1.HORIZONTAL_SPANS[w.state.hIndex] || GridSpans_1.HORIZONTAL_SPANS[5],
                    vSpan: w.state.vSpan || GridSpans_1.VERTICAL_SPANS[w.state.vIndex] || GridSpans_1.VERTICAL_SPANS[3]
                }));
        ...
        if (!currentActiveState.hSpan) {
            currentActiveState.hSpan = GridSpans_1.HORIZONTAL_SPANS[currentActiveState.hIndex] || GridSpans_1.HORIZONTAL_SPANS[5];
        }
        if (!currentActiveState.vSpan) {
            currentActiveState.vSpan = GridSpans_1.VERTICAL_SPANS[currentActiveState.vIndex] || GridSpans_1.VERTICAL_SPANS[3];
        }
        ...
        const normalizedWindows = activeWindows.map(w => {
            const state = { ...w.state };
            if (!state.hSpan) {
                state.hSpan = GridSpans_1.HORIZONTAL_SPANS[state.hIndex] || GridSpans_1.HORIZONTAL_SPANS[5];
            }
            if (!state.vSpan) {
                state.vSpan = GridSpans_1.VERTICAL_SPANS[state.vIndex] || GridSpans_1.VERTICAL_SPANS[5];
            }
