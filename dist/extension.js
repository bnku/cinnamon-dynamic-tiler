...
class DynamicTilerExtension {
    ...
    verticalGridColumns;
    verticalGridRows;
    ultrawideGridColumns;
    ultrawideGridRows;
    monitorGridOverrides;
    minSpan;
    minColumnSpan;
    minRowSpan;
    step;
    ...
    indexAllWindows(activeMonitor, config, monitors, forceAddNew = false) {
        ...
        for (const id of visibleWindowIds) {
            ...
            try {
                ...
                const gridColumns = (0, types_1.getGridColumns)(currentConfig);
                const gridRows = (0, types_1.getGridRows)(currentConfig);
                const minColumnSpan = (0, types_1.getMinColumnSpan)(currentConfig);
                const minRowSpan = (0, types_1.getMinRowSpan)(currentConfig);
                ...
                if (cached) {
                    ...
                    const cachedFitsGrid = cached.state.hSpan[0] >= 0 &&
                ...
                if (!isResting) {
                    ...
                    hSpan = TilingEngine_1.TilingEngine.geometryToHSpan(currentVisible, currentMonitor, currentConfig);
                    vSpan = TilingEngine_1.TilingEngine.geometryToVSpan(currentVisible, currentMonitor, currentConfig);
                ...
                if (!cached && forceAddNew) {
                    ...
                    const snappedGeom = TilingEngine_1.TilingEngine.stateToGeometry(candidateState, currentMonitor, currentConfig);
    ...
    onDragUpdate() {
        ...
        try {
            ...
            const isSwapModifierPressed = this.experimentalSwapSameShapeWindows === true &&
            ...
            if (this.dragSession &&
                ...
                (0, DragTiling_1.shouldCancelSourceReturn)(this.dragSession.sourceState, dragTarget.targetHSpan, dragTarget.targetVSpan, dragTarget.intentPoint)) {
                ...
                try {
                    ...
                    const win = this.shell._findMetaWindow(this.draggedWindowId);
                    if (win) {
                        const frameGeom = TilingEngine_1.TilingEngine.stateToGeometry(this.dragSession.sourceState, activeMonitor, config);
    ...
    logDndDecision(dragTarget, dragResult, windowWidth, windowHeight, windowSizeSource, activeWindows) {
        ...
        const verticalGroup = debug.verticalGroup
            ...
            : '-';
        this.debugLog(`[Dynamic Tiler] DnD trace status=${dragResult.status}` +
        ...
        try {
            ...
            for (const id of visibleWindowIds) {
                ...
                try {
                    ...
                    const hSpan = TilingEngine_1.TilingEngine.geometryToHSpan(currentGeom, monitor, config);
                    const vSpan = TilingEngine_1.TilingEngine.geometryToVSpan(currentGeom, monitor, config);
                    const nextState = {
                        ...cached.state,
                        hSpan,
                        vSpan,
