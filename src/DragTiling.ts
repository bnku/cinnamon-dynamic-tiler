...
function spansEqual(spanA: [number, number] | undefined, spanB: [number, number] | undefined): boolean {
  return Boolean(spanA && spanB && spanA[0] === spanB[0] && spanA[1] === spanB[1]);
}

export function shouldFloatAfterModifierRelease(input: ExtractionIntentInput): boolean {
...
export function computeDragTarget(input: DragTargetInput): DragTargetResult {
  ...
  let targetHSpan: [number, number] = [startCol, startCol + targetWidth];
  ...
  const stackGroups = new Map<string, { hSpan: [number, number]; windows: typeof stackWindows }>();
  ...
  const stackGroupCandidates = Array.from(stackGroups.values())
    ...
    .sort((a, b) =>
      Number(b.containsCursor) - Number(a.containsCursor) ||
  ...
  const horizontalGroups = new Map<string, { vSpan: [number, number]; windows: typeof stackWindows }>();
  for (const w of allStackWindowCandidates) {
  ...
  if (cursorHorizontalGroup && !shouldPreferVerticalStack) {
    ...
    const adjacentWindows = cursorHorizontalGroup.windows
      .filter(w =>
        Math.abs(w.state.hSpan[0] - nearestBoundary) <= 1 ||
    ...
    const isWideTargetClampedToEdge =
...
export function calculateDragTransitions(
  ...
  const tryRelocateTightVerticalStack = (): boolean => {
    ...
    const candidateHSpans: [number, number][] = [];
    ...
    const overlapsMovedStack = (candidateStates: Record<string, WindowState>, state: WindowState) => {
    ...
    const carveAwayFromHSpan = (state: WindowState, avoidHSpan: [number, number]): WindowState | null => {
      const avoidCenter = spanCenter(avoidHSpan);
      const stateCenter = spanCenter(state.hSpan);
      const candidates: [number, number][] = avoidCenter >= stateCenter
  ...
  const tryRelocateTightHorizontalStack = (): boolean => {
    ...
    const isAutoNarrowedScreenEdgeInsertion =
      Boolean(options.preferredWidth && options.preferredWidth > targetWidth) &&
  ...
  const tryCarveEdgeInsertionCorridor = (): boolean => {
    ...
    const isExplicitScreenEdgeInsertion =
      ...
      (targetTouchesRightEdge && intentNearRightEdge);
    const isAutoNarrowedScreenEdgeInsertion =
      Boolean(options.preferredWidth && options.preferredWidth > spanSize(targetHSpan)) &&
      (targetTouchesLeftEdge || targetTouchesRightEdge);
