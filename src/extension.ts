...
class DynamicTilerExtension {
  ...
  // DnD State tracking
  private grabBeginId: number = 0;
  private grabEndId: number = 0;
  private draggedWindow: any = null;
  private draggedWindowId: string = '';
  private dragTimerId: number = 0;
  private previewsMap: Record<string, TilePreview> = {};
  private lastDragStates: Record<string, WindowState> | null = null;
  private lastDragMonitor: any = null;
  ...
  public enable(): void {
    try {
      ...
      this.grabBeginId = global.display.connect('grab-op-begin', (...args: any[]) => {
  ...
  private onGrabBegin(window: any, op: any): void {
  ...
  private indexAllWindows(activeMonitor: any, config: any, monitors: any, forceAddNew: boolean = false): void {
    ...
    const confirmedSpansMap: Record<string, { hSpan: [number, number]; vSpan: [number, number] }[]> = {};
    ...
    try {
      ...
      if (dragResult.status === 'blocked') {
        ...
        try {
          ...
          const win = (this.shell as any)._findMetaWindow(this.draggedWindowId);
          if (win) {
            const blockedState: WindowState = {
      ...
      if (session.lastDragStates) {
        ...
        for (const [id, nextState] of Object.entries(session.lastDragStates)) {
          try {
            ...
            const nextStateTyped = nextState as WindowState;
  ...
  private logDndDecision(
    dragTarget: DragTargetResult,
    dragResult: DragSolveResult,
    windowWidth: number,
    windowHeight: number,
  ...
  private formatSpan(span: [number, number]): string {
    return `${span[0]}-${span[1]}`;
  }

  private formatDndWindows(windows: { windowId: string; state: WindowState }[]): string {
    return windows
  ...
  private clearPlacementPreviews(): void {
  ...
  private clearBlockedPreview(): void {
    if (this.blockedPreview) {
      try {
