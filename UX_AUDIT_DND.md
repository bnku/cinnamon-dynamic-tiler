...
- extraction теперь умеет возвращать сжатого DnD-соседа назад через transaction journal и защищена threshold-ом от случайного отпускания modifier, но пока хранит только последнюю DnD-транзакцию в памяти;
- вертикальный stack insertion теперь покрыт базовыми cursor-to-target тестами, умеет horizontal relief для узкого pinned stack-а и получил hysteresis, но все еще требует ручной обкатки в плотных real-world раскладках;
- experimental swap уже добавлен, но его нужно обкатать руками и решить, стоит ли делать его дефолтным.

Оценка DnD UX после изменений: примерно 8.4/10. Это уже рабочая и приятная база, но в плотных раскладках с несколькими соседями пользователь все еще может увидеть "магическое" решение.
...
Запуск `npm test -- --runInBand` прошел успешно: 75/75 тестов зеленые.
...
- multi-step transaction history, если между insertion и extraction было несколько DnD-операций;
