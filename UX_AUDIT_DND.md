...
- есть базовый визуальный blocked preview и solver result с причинами `wouldOverlap/tooSmall/outOfBounds`, но UI пока не показывает причину пользователю;
- extraction теперь умеет возвращать сжатого DnD-соседа назад через transaction journal, но пока хранит только последнюю DnD-транзакцию в памяти;
- вертикальный stack insertion теперь покрыт базовыми cursor-to-target тестами и умеет horizontal relief для узкого pinned stack-а, но все еще требует ручной обкатки в плотных real-world раскладках;
- experimental swap уже добавлен, но его нужно обкатать руками и решить, стоит ли делать его дефолтным.

Оценка DnD UX после изменений: примерно 7.5/10. Это уже рабочая и приятная база, но в плотных раскладках с несколькими соседями пользователь все еще может увидеть "магическое" решение.
...
Ограничение: сейчас хранится только последняя DnD-транзакция. Это осознанный UX-safe минимум: он закрывает самый частый сценарий "вставил и сразу вынул", но не пытается делать сложный multi-step undo layout-а.
...
Статус после итерации: добавлен guard `hasLayoutOverlaps`; extension не коммитит результат, если итоговые states пересекаются. После второй итерации вместо полного исчезновения preview показывается красная dashed-рамка заблокированного target-а, а solver возвращает `status/reason/affected`.
...
Следующий слой: UI может использовать `reason`, чтобы различать overlap, недостаточный `minSpan` и выход за границы, а не просто показывать общий blocked state.
...
Запуск `npm test -- --runInBand` прошел успешно: 60/60 тестов зеленые.
...
Статус: частично реализовано. Добавлен `solveDragTransitions`:
...
Extension уже использует этот результат для blocked preview и запрета commit-а. Следующий слой - сделать визуальную причину блокировки различимой для пользователя, если Cinnamon UI позволит сделать это аккуратно.
...
Самые приоритетные оставшиеся улучшения: более глубокие permutation-тесты, визуальное различение причин blocked state, расширение transaction journal за пределы последнего commit-а и ручная обкатка experimental swap. После этого DnD станет не просто рабочим, а таким, которому начинаешь доверять рукой.
