...
- target под курсором пока выбирается эвристиками, а не полноценным intent solver;
- часть решений все еще зависит от порядка окон;
- нет явного invalid/blocked preview;
- схлопывание вакансии во время preview и после floating-out живет двумя разными алгоритмами;
- вертикальный stack insertion появился, но пока не покрыт тестами на уровне cursor-to-target логики;
- нет отдельного swap-сценария для очевидного обмена местами равноценных окон.
...
Рекомендация: `calculateDragTransitions` должен возвращать не только states, но и статус:
...
Если валидного решения нет без неприятного сжатия соседей, показывать disabled preview и запрещать commit. Это лучше, чем молча двигать пол-экрана.

### 5. Результат все еще может зависеть от порядка окон
...
Сама математика "вставить между двумя vertical windows" покрыта unit-тестом через готовый `targetVSpan`. Но пользовательский слой, который превращает координаты курсора в этот `targetVSpan`, живет внутри `extension.ts` и не тестируется отдельно.
...
Рекомендация: вынести `computeDragTarget(mx, my, sourceState, activeWindows, monitor, config)` в pure helper и покрыть его тестами. Это сделает UX-эвристики обсуждаемыми и безопасно изменяемыми.
...
### 9. Нет простого swap для равноценных окон
...
Рекомендация: добавить swap как отключаемую экспериментальную функцию в настройках расширения.
...
Запуск `npm test -- --runInBand` прошел успешно: 35/35 тестов зеленые.
...
- cursor-to-target расчет в `extension.ts`;
...
Вынести расчет target из `extension.ts` в чистую функцию и покрыть тестами:
...
Это самый важный шаг для UX, потому что сейчас именно target intent является источником большинства "а почему оно решило так?".
...
Это позволит UI показывать нормальный preview, disabled preview или вообще не commit-ить опасный результат.

### Этап 3: Убрать order-dependence

Добавить deterministic sorting и property-style regression tests:
...
Добавить отключаемую настройку расширения: `experimentalSwapSameShapeWindows`.
...
- two non-neighbor same-shape windows swap places;
- disabling setting returns to normal solver behavior;
- same area but different shape does not swap;
- cursor near target edge does not swap and can trigger insertion/carve;
...
Самые приоритетные улучшения: pure `computeDragTarget`, единый vacancy collapse, invalid preview, deterministic ordering, experimental same-shape swap и transaction journal для extraction. После этого DnD станет не просто рабочим, а таким, которому начинаешь доверять рукой.
