...
Дата повторного аудита: 2026-05-31

Фокус: UX перетаскивания окон мышью в Cinnamon extension после последних правок. Дополнительно отмечены смежные keyboard-tiling правки, которые влияют на доверие к той же grid-математике. Аудит сделан по коду и unit-тестам, без live GUI-тестирования.

## Статус Итерации 2026-05-31

Выполнена первая инженерная итерация по этому плану:

- добавлен первый experimental swap одинаковых по форме tiled-окон;
- на тот момент swap был вынесен в настройку `experimentalSwapSameShapeWindows` и выключен по умолчанию;
- на тот момент swap срабатывал только при центральном попадании в target-окно, чтобы не конфликтовать с edge insertion/carving;
...
Выполнена десятая инженерная итерация:

- experimental swap переведен из неявного center-drop в отдельный modifier-driven режим;
- обычный DnD без swap modifier больше не пытается свапать окна и остается insertion/carve/push жестом;
- explicit swap теперь разрешает обмен соседних окон, не-соседних окон и окон разного размера;
- для swap добавлена отдельная настраиваемая клавиша `dnd-swap-modifier-key`, default `<Control><Shift>d`;
- swap preview получил отдельные зеленые primary/secondary рамки, чтобы пользователь видел именно режим обмена, а не обычную вставку;
- добавлены property-style permutation тесты для edge corridor insertion и full-height vertical stack redistribution;
- исправлен тестовый permutation-helper, чтобы он проверял настоящие перестановки без дублирования окон.

Выполнена одиннадцатая инженерная итерация:

- исправлен смежный keyboard-tiling UX-баг `Super+стрелок`, который проявился после ручного изменения ширины окна;
- старый fallback при заблокированном расширении резко схлопывал активное окно до половины или `minSpan`;
- новый fallback делает мягкое пошаговое сжатие противоположной стороны на `step`, сохраняя понятный контракт "одно нажатие - один слот";
- если `Super+влево` не может расширить окно влево из-за плотной левой цепочки, окно уменьшается справа, а правый сосед занимает освобожденный слот;
- если `Super+вправо` не может расширить окно вправо из-за правого упора, окно уменьшается слева, а левый сосед занимает освобожденный слот;
- правило сделано симметричным для вертикальной оси (`up/down`);
- добавлены regression-тесты на оба горизонтальных сценария с активным широким окном между плотной левой цепочкой и правым соседом;
- обновлен старый тестовый контракт, который раньше закреплял резкое схлопывание как допустимое поведение.
...
Базовый аудит-план по DnD почти закрыт. До "конфетки" осталось несколько важных UX-слоев:
...
- самые рискованные решения покрыты permutation-тестами и базовым deterministic property/fuzz слоем; дальше нужен расширенный fuzz на extraction/swap/keyboard и больше классов плотных раскладок;
...
- explicit swap уже добавлен как отдельный modifier-driven режим; его нужно обкатать руками и решить, стоит ли включать setting по умолчанию.
- смежная keyboard-математика получила мягкий blocked-resize fallback и regression-тесты, но ее тоже стоит включить в расширенный fuzz, потому что DnD и hotkeys работают поверх одной mental model сетки.

Оценка DnD UX после изменений: примерно 9.1/10. Это уже рабочая и приятная база: появился первый global layout scoring для выбора из нескольких валидных carve/relocate-кандидатов, а property/fuzz начал защищать базовые инварианты. Следующий скачок качества должен прийти от расширения этого scoring-а на все multi-window решения, а не от очередной локальной эвристики.
...
### 1. Intent-aware carving нужно расширить на global layout scoring
...
Текущая рекомендация: поднять scoring на уровень всего layout-кандидата.
...
### 2. Source vacancy preview и final floating collapse

Статус: базовый UX-разрыв закрыт. Preview-схлопывание source vacancy теперь использует тот же `collapseVacancy`, что и настоящий floating-out.

Оставшийся риск: в сложных multi-window сценариях сам `collapseVacancy` все еще выбирает кандидата локально. Поэтому улучшение здесь теперь совпадает с global scoring: меньше локальных эвристик, больше общей стоимости layout-кандидата.

Текущая рекомендация: держать один collapse-модуль для:

- для preview;
- для floating-out;
- для тестов;
- для будущего cross-monitor source collapse.

### 3. Extraction знает, кого сжало конкретное окно

Статус: закрыто в UX-safe форме через bounded transaction history. При extraction extension сначала пытается восстановить именно тех соседей, которых это dragged-окно сжало при DnD commit-е.

Исторически проблемный пример:
...
- solver мог искать безопасное заполнение вакансии, потому что не хранил историю "этот терминал сжал именно Chrome".

При commit DnD сохраняется:
...
Так пользователь получает ощущение "если я вынул то, что только что вставил, раскладка чинится обратно", но без опасного blind undo.
...
### 4. Визуальный invalid preview есть, но без текстового объяснения

Статус: базовый риск закрыт. Solver возвращает `valid/blocked`, причину блокировки и affected windows; extension не коммитит конфликтный layout и показывает blocked preview.
...
### 5. Tie-сценарии стали стабильнее, но требуют fuzz-нагрузки

Статус: для ключевых DnD-веток добавлена детерминированная сортировка по grid-позиции и stable id. Edge corridor insertion и full-height vertical stack redistribution уже проверяются permutation-тестами.

Оставшийся риск: текущие permutation-тесты покрывают самые болезненные ветки, но не перебирают большие случайные layout-ы. Одинаковая геометрия не должна менять результат после перезапуска Cinnamon, смены focus order или переиндексации cache.
...
- расширить regression tests до property/fuzz набора, где один и тот же layout подается в разных permutation и дает одинаковый результат.

### 6. Vertical stack insertion стал рабочим, но его нужно нагружать плотными раскладками
...
Статус после девятой итерации: pure-helper добавлен, extension использует его напрямую, stack-boundary hysteresis есть, full-height stack умеет redistribution in-place, локальный pinned stack умеет horizontal relief.

Следующий слой: расширить property/fuzz тесты на неоднозначные пересечения двух stack-ов, неравные высоты соседей и дребезг около границ.
...
### 9. Experimental swap для явного обмена окон добавлен

Пользователь часто мыслит не "вставить окно в сетку", а "поменять эти два окна местами". Чтобы это не конфликтовало с обычной вставкой и расталкиванием, swap вынесен в отдельный modifier-driven режим.

Пограничный пример:

- terminal A занимает `[0,2] x [0,6]`;
- terminal B занимает `[8,12] x [6,12]`;
- пользователь тащит A на B с зажатым swap modifier;
- ожидаемый результат: A и B меняются местами, остальные окна не участвуют.

Статус после итерации: swap добавлен как отключаемая экспериментальная функция `experimentalSwapSameShapeWindows`, по умолчанию выключен. Сам жест активируется отдельным `dnd-swap-modifier-key`.

Контракт срабатывания:

- настройка `experimentalSwapSameShapeWindows` включена;
- зажат настроенный swap modifier, default `<Control><Shift>d`;
- курсор находится над tiled target window;
- dragged и target могут быть соседями или не соседями;
- dragged и target могут иметь разную ширину/высоту;
- без swap modifier этот же жест возвращается к обычному DnD solver-у.
...
- dragged landing slot и target return slot подсвечиваются зеленой swap-рамкой;
- target window получает preview переезда в source slot/shape dragged-окна;
...
Запуск `npm test -- --runInBand` прошел успешно: 86/86 тестов зеленые.
...
- explicit swap для соседних, не-соседних и разных по форме окон;
- отключение swap без отдельного swap modifier-а;
- keyboard blocked-resize fallback: активное окно мягко сжимается с противоположной стороны, если расширение в сторону стрелки заблокировано плотной цепочкой.
- базовый property/fuzz слой: сгенерированные плотные 12x12 layout-ы не дают valid-результатам выйти за grid, нарушить `minSpan` или создать overlap;
- детерминизм DnD solve-result на fuzz-layout-ах при разных permutation входного списка окон;
- первый global scoring слой для выбора между валидными carve/relocate-кандидатами по общей стоимости раскладки: affected windows, движение, потеря/изменение площади, валидность.
...
- расширенные property/fuzz тесты на extraction, explicit swap, keyboard resize и cross-axis consistency;
- global scoring для всех multi-window решений, включая общий push/collapse path, а не только carve/relocate ветки;
- explicit swap: ручная обкатка modifier-а и preview в реальном Cinnamon;
- cross-axis consistency между DnD и keyboard resize в редких плотных раскладках.
...
Статус: реализовано для ключевых DnD-веток. Добавлена deterministic sorting по grid-позиции и stable id, плюс regression/property-style тесты на несколько permutation входных окон.
...
Дополнительно покрыты permutation-сценарии для двух самых рискованных веток:

- edge corridor insertion, где цепочка минимальных колонок сдвигается, а первый широкий donor сжимается;
- vertical stack redistribution, где full-height stack перераспределяется по высоте вместо горизонтального relief-а.

### Этап 4: Experimental explicit swap

Статус: реализовано. Добавлена отключаемая настройка расширения `experimentalSwapSameShapeWindows` и отдельный настраиваемый modifier `dnd-swap-modifier-key`.

UX-контракт:

1. Без swap modifier работает обычный DnD: magnetic zones, stack insertion, free slot, carve/push.
2. С включенным setting и зажатым swap modifier drop на tiled-окно означает swap.
3. Swap меняет окна местами вместе с размерами, поэтому работает и для соседей, и для окон разной формы.
4. Swap preview подсвечивает два участника отдельной зеленой рамкой.

Покрытые и желательные тесты:

- two non-neighbor same-shape windows swap places - сделано;
- disabled swap modifier returns to normal solver behavior - сделано;
- different-shape windows swap places - сделано;
- cursor near target edge still swaps when modifier is active - сделано;
- neighboring different-shape windows swap places - сделано;
- swap preview affects only dragged and target windows - сделано на визуальном уровне через отдельный preview variant.

Это дает пользователю очень понятный контракт: "держу swap modifier - меняю окна местами"; обычная вставка при этом не теряет предсказуемость.
...
Статус: реализовано на базовом UX-safe уровне. Фича Ctrl/modifier extraction оставлена, но стала безопаснее:
...
### Этап 7: Keyboard/DnD consistency

Статус: добавлена первая важная правка. Keyboard resize теперь не использует резкое аварийное схлопывание при заблокированном расширении. Вместо этого активное окно сжимается на один `step` с противоположной стороны, а соседняя цепочка подтягивается в освободившееся место.

UX-контракт:

1. `Super+влево` сначала пытается расширить окно влево.
2. Если слева плотный упор, активное окно уменьшается справа на один слот.
3. `Super+вправо` зеркально уменьшает окно слева, если расширение вправо заблокировано.
4. То же правило применяется к `up/down`.
5. Одно нажатие не должно внезапно менять окно на половину экрана или `minSpan`, если пользователь не делал серию таких нажатий.

Следующий слой: добавить property/fuzz проверки, что keyboard resize и DnD не создают overlap, не нарушают `minSpan` и не двигают окна без геометрической причины.

### Этап 8: Property/fuzz и global scoring

Статус: базовый слой выполнен, следующий шаг - расширение покрытия и перенос scoring-а на оставшиеся solver-ветки.

Что уже сделано:

- добавлен deterministic generator валидных плотных 12x12 layout-ов;
- fuzz проверяет, что valid DnD-result не создает overlap, не выходит за grid и не нарушает `minSpan`;
- fuzz проверяет стабильность результата при разных permutation входного списка окон;
- добавлен общий scoring helper для layout-кандидатов;
- carve/relocate ветки теперь выбирают не первый валидный вариант, а лучший по общей стоимости изменения layout-а.

Оставшаяся цель property/fuzz слоя:

- прогонять extraction, swap и keyboard resize на разных permutation входных окон;
- проверять invariants: нет overlap, нет out-of-bounds, spans не меньше `minSpan`, результат детерминирован, preview/commit совпадают по affected-окнам;
- отдельно нагружать крайние случаи: pinned edge stacks, full-height vertical stacks, tight corridors, wide dragged windows, ручной resize перед hotkey.

Оставшаяся цель global layout scoring:

- расширить сравнение валидных layout-кандидатов на общий push/collapse path;
- штрафовать лишнее движение third-party окон, потерю площади, смену оси без явного intent-а и неожиданное вовлечение окон вне зоны жеста;
- предпочитать решения, где affected-набор минимален и совпадает с тем, что пользователь видел в preview;
- сделать результат более похожим на "тайлер понял мой жест", а не "тайлер нашел какой-то математически валидный выход".

## Итог

Текущая реализация уже закрыла главную боль: DnD больше не выглядит как непредсказуемая перестройка всей сетки. Смежный keyboard resize тоже стал ближе к тому же UX-контракту: маленький жест дает маленькое, предсказуемое изменение. Следующий качественный скачок будет не от еще одной локальной эвристики, а от явного UX-контракта:
...
Самые приоритетные оставшиеся улучшения: расширенный property/fuzz на extraction/swap/keyboard, global layout scoring для общего push/collapse path и ручная обкатка explicit swap. После этого DnD станет не просто рабочим, а таким, которому начинаешь доверять рукой.
